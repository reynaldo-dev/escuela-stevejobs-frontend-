...
export const Notas = () => {
  ...
  const loadAlumnoInfo = async () =>{
    const res = await fetchWithtoken({}, 'GET', `alumno/${user?._id}`)
    const data = await res.json()
   return data
  }
      ...
      <Container className={classess.mainContainer}>
        ...
        <Box className={classess.tableContainer}>
...

          {
            ...
            <NotasPDF notas={data ? data?.data?.primer_periodo : ''} user={user} periodo={ alumno?.data[0].nombre == 'Bachillerato general' ? 'Periodo 1' : 'Primer trimestre' } />
          </PDFViewer>
        ...
        <Box className={classess.tableContainer}>
...

          {
            openNotasP2 &&
            <PDFViewer height={1000} width={1000} style={{margin:'0 auto'}} >
                         <NotasPDF notas={data ? data?.data?.segundo_periodo : []} user={user} periodo={ alumno?.data[0].nombre == 'Bachillerato general' ? 'Periodo 2' : 'Segundo trimestre' } />
        ...
        <Box className={classess.tableContainer}>
...

          {
            openNotasP3 &&
            <PDFViewer height={1000} width={1000} style={{margin:'0 auto'}} >
                          <NotasPDF notas={data ? data?.data?.tercer_periodo : []} user={user} periodo={ alumno?.data[0].nombre == 'Bachillerato general' ? 'Periodo 3' : 'Tercer trimestre' } />
...

        {
          ...
          <Box className={classess.tableContainer}>
...

            {
              openNotasP4 &&
              <PDFViewer height={1000} width={1000} style={{ margin: '0 auto' }} >
                <NotasPDF notas={data ? data?.data?.cuarto_periodo : []} user={user} periodo='Periodo 4' />
